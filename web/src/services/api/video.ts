...
async function pollOpenAIVideoTask(config: AiConfig, task: VideoGenerationTask, options?: RequestOptions): Promise<VideoGenerationTaskState> {
    try {
        const video = unwrapVideoResponse((await axios.get<ApiVideoResponse>(aiApiUrl(config, `/videos/${task.id}`), { headers: { ...aiHeaders(config), ...(config.serverManaged ? { "x-ai-method": "GET" } : {}) }, signal: options?.signal })).data);
        if (video.status === "completed") {
            const content = await axios.get<Blob>(aiApiUrl(config, `/videos/${task.id}/content`), { headers: { ...aiHeaders(config), ...(config.serverManaged ? { "x-ai-method": "GET" } : {}) }, responseType: "blob", signal: options?.signal });
...
async function pollSeedanceTask(config: AiConfig, task: VideoGenerationTask, options?: RequestOptions): Promise<VideoGenerationTaskState> {
    try {
        const state = unwrapSeedanceTask((await axios.get<ApiEnvelope<SeedanceTask>>(seedanceApiUrl(config, task.id), { headers: { ...aiHeaders(config), ...(config.serverManaged ? { "x-ai-method": "GET" } : {}) }, signal: options?.signal })).data);
...
function assertVideoConfig(config: AiConfig, model: string) {
    ...
    if (!config.apiKey.trim() && !config.serverManaged) throw new Error("请先配置 API Key");
