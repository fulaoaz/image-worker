...
const imageLogStore = localforage.createInstance({ name: "image-worker", storeName: "image_generation_logs" });
const videoLogStore = localforage.createInstance({ name: "image-worker", storeName: "video_generation_logs" });
...
async function syncDomain<T>(config: WebdavSyncConfig, onProgress: AppSyncProgress | undefined, options: SyncDomainOptions<T>): Promise<SyncDomainResult<T>> {
    try {
        ...
        const manifest: DomainManifest<T> = { app: "image-worker", version: 1, domain: options.key, exportedAt: new Date().toISOString(), data: mergedData, files: uploaded.files };
...
async function readDomainManifest<T>(config: WebdavSyncConfig, domain: DomainKey, emptyData: T): Promise<DomainManifest<T> | null> {
    ...
    if (data.app !== "image-worker" || data.domain !== domain) throw new Error(`${domain} 同步清单不是当前应用的数据`);
    return {
        app: "image-worker",
        version: 1,
        domain,
