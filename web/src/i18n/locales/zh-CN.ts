export default {
    ...
    home: {
        promptError: "获取提示词失败",
        description: "在 <canvas>Image Worker</canvas> 中生成、连接和重组 <content>图片、文字与图形</content>，让创作从单次生成变成连续推演。",
        start: "开始使用",
        openCanvas: "打开画布",
    ...
    config: {
        ...
        localStorage: {
            ...
            description: "查看 Image Worker 在浏览器中保存的数据量，并按对象仓库统计内容体积。",
        ...
        channels: {
            ...
            serverTitle: "服务端渠道",
            serverDescription: "由管理员在服务器配置的渠道，API Key 只由服务端保存。刷新后即可在模型选择器中使用。",
            serverRefresh: "拉取服务端渠道",
            serverManaged: "服务端托管",
            serverReady: "可直接使用",
            serverLoading: "正在拉取服务端渠道…",
            serverEmpty: "当前没有可用的服务端渠道。",
            serverLoaded: "已拉取 {{count}} 个服务端渠道",
            serverLoadFailed: "服务端渠道拉取失败",
        },
        preferences: {
            interface: "界面偏好",
            language: "界面语言",
    ...
    admin: {
        eyebrow: "IMAGE WORKER 管理",
        title: "模型渠道服务端配置",
        description: "渠道和 API Key 仅保存在服务器数据目录。普通用户刷新页面后可使用已启用的模型，但不会收到 API Key。",
        back: "返回用户端",
        token: "管理员 Token",
        tokenDescription: "仅用于本次读取或保存，不会保存在浏览器中。",
        tokenPlaceholder: "服务器环境变量 ADMIN_TOKEN",
        load: "读取配置",
        reload: "重新读取",
        save: "保存到服务器",
        tokenRequired: "请先填写管理员 Token",
        loadFailed: "读取服务器配置失败",
        loaded: "已读取服务器配置",
        saveFailed: "保存服务器配置失败",
        saved: "服务器配置已保存，普通用户刷新后即可使用",
        defaultChannel: "服务器渠道",
        channelNumber: "渠道 {{count}}",
        providers: "模型渠道",
        providerDescription: "已就绪 {{configured}} / {{total}} 个；渠道内的模型会按能力显示给用户。",
        add: "新增渠道",
        ready: "已就绪",
        draft: "待完善",
        empty: "还没有渠道，新增一个后在右侧抽屉中填写接口和模型。",
    },
    agent: {
        ...
        connect: { pluginTitle: "方式一：在 Codex 中使用插件", pluginText: "在 Codex app 安装 Image Worker 插件后，通过插件启动画布，插件会自动启动本地 Agent 并带上连接信息。", directTitle: "方式二：直接运行 Agent", directText: "不使用 Codex 插件时，在终端运行下面命令，再回到网页里连接或手动填入 Local URL 和 Connect token。", commandCopied: "命令已复制", pluginReminder: "Codex 插件提醒", pluginReminderText: "只有安装 Codex 插件或手动添加 MCP 后，工具列表才会进入 Codex 上下文并增加 token 消耗；仅运行 npx -y @fulaoaz/canvas-agent 启动本地 Agent 不会安装 MCP。", removePlugin: "移除插件", removeMcp: "移除手动 MCP", copyCommand: "复制命令", title: "连接本地 Agent", description: "按使用场景选择一种连接方式。", webConnection: "网页连接", autoDiscover: "默认自动读取 Local URL 和 Connect token，失败时再手动填写。", disconnect: "断开", connect: "连接", localAddress: "本地地址", urlPlaceholder: "例如 http://127.0.0.1:17371", token: "连接 Token", tokenPlaceholder: "自动发现，或手动填入 Connect token" },
