...
export function CanvasNodeMaskEditDialog({ dataUrl, open, onClose, onConfirm }: { dataUrl: string; open: boolean; onClose: () => void; onConfirm: (payload: CanvasImageMaskEditPayload) => void }) {
    ...
    const startDraw = (event: ReactPointerEvent<HTMLCanvasElement>) => {
        ...
        if (maskCanvasRef.current && previewCanvasRef.current) renderMaskPreview(maskCanvasRef.current, previewCanvasRef.current);
        draw(event);
    };
    ...
    const stopDraw = () => {
        ...
        const maskCanvas = maskCanvasRef.current;
        const previewCanvas = previewCanvasRef.current;
        if (maskCanvas && previewCanvas) renderMaskPreview(maskCanvas, previewCanvas, canvasHasPaint(maskCanvas));
    };
