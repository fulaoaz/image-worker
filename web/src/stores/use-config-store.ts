...
export async function fetchServerModelChannels() {
    ...
    const payload = (await response.json()) as { channels?: ModelChannel[] };
    return (payload.channels || []).map((channel) => createModelChannel({ ...channel, serverManaged: true }));
}
...
export function useEffectiveConfig() {
    ...
    useEffect(() => {
        void fetchServerModelChannels().then(setServerChannels).catch(() => setServerChannels([]));
    }, []);

    return useMemo(() => {
        const localChannels = config.channels.filter((channel) => !channel.serverManaged);
        if (!serverChannels.length) return localChannels.length ? withModelChannels({ ...config, channelMode: "local" as const }, localChannels) : { ...config, channelMode: "local" as const };
        const serverChannelIds = new Set(serverChannels.map((channel) => channel.id));
        return withModelChannels({ ...config, channelMode: "remote" as const }, [...serverChannels, ...localChannels.filter((channel) => !serverChannelIds.has(channel.id))]);
    }, [config, serverChannels]);
}

export function withModelChannels(config: AiConfig, channels: ModelChannel[]): AiConfig {
    const normalizedChannels = channels.map((channel) => createModelChannel(channel));
    const models = modelOptionsFromChannels(normalizedChannels);
    return {
        ...config,
        channelMode: normalizedChannels.some((channel) => channel.serverManaged) ? "remote" : "local",
        ...
        channelId: normalizedChannels[0]?.id || config.channelId,
        serverManaged: Boolean(normalizedChannels[0]?.serverManaged),
        imageModel: normalizeDefaultModelValue(config.imageModel, filterModelsByCapability(models, "image"), normalizedChannels),
        videoModel: normalizeDefaultModelValue(config.videoModel, filterModelsByCapability(models, "video"), normalizedChannels),
        textModel: normalizeDefaultModelValue(config.textModel, filterModelsByCapability(models, "text"), normalizedChannels),
        audioModel: normalizeDefaultModelValue(config.audioModel, filterModelsByCapability(models, "audio"), normalizedChannels),
        imageModels: selectModelList(config.imageModels, models, "image", normalizedChannels),
        videoModels: selectModelList(config.videoModels, models, "video", normalizedChannels),
        textModels: selectModelList(config.textModels, models, "text", normalizedChannels),
        audioModels: selectModelList(config.audioModels, models, "audio", normalizedChannels),
    };
}
