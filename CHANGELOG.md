...
+ [调整] GitHub Actions 在 `main` 分支构建镜像时同步推送 `latest` 标签，便于部署自动更新。
+ [新增] 图片节点支持转可编辑 SVG 并在线可视化编辑，描摹在本机浏览器完成。
+ [新增] 可编辑 SVG 转换新增本机文字识别图层，识别失败不影响矢量结果。
+ [新增] Docker 部署新增 `/admin` 服务端模型渠道管理页，API Key 仅保存在容器数据目录并由服务端代理转发。
+ [新增] 用户配置页新增服务端渠道区域和手动刷新按钮，可查看管理员提供的模型并直接用于模型选择。
