...
export function checkVersions() {
    ...
    if (!localCodexVersion) {
        ...
        logger.warn(`Bundled Codex ${CODEX_VERSION} does not match local Codex ${localCodexVersion}. Keep both current with: npm install -g @openai/codex@latest && npx -y @fulaoaz/canvas-agent@latest`);
    }
...
async function checkLatestVersions(localCodexVersion: string) {
    try {
        const [latestAgent, latestCodex] = await Promise.all([
            npmVersion("@fulaoaz/canvas-agent"),
            npmVersion("@openai/codex"),
        ]);
        if (isOlder(VERSION, latestAgent)) logger.warn(`Update available: Canvas Agent ${VERSION} -> ${latestAgent}. Run: npx -y @fulaoaz/canvas-agent@latest`);
        if (isOlder(CODEX_VERSION, latestCodex)) logger.warn(`Update available: bundled Codex ${CODEX_VERSION} -> ${latestCodex}. Upgrade Canvas Agent with: npx -y @fulaoaz/canvas-agent@latest`);
